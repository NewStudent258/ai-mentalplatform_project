<template>
    <div>
        ...
        <el-alert type="info" :closable="false" show-icon class="page-notice">
          <template #title>
            列表默认按心情分升序排列——状态最差的记录排在最前，便于优先关注。<br />
            情绪日记是学生的私人记录，本页仅供查看与评估，不提供删除操作。
          </template>
        </el-alert>
        ...
        <el-table :data="tableData" style="width: 100%">
            ...
            <el-table-column label="主要情绪" width="110">
                <template #default="scope">{{ scope.row.dominantEmotion || '—' }}</template>
            </el-table-column>
            ...
            <el-table-column prop="emotionTriggers" label="情绪触发因素" width="160" show-overflow-tooltip />
            <el-table-column prop="diaryContent" label="日记内容" min-width="220" show-overflow-tooltip />
            <el-table-column label="操作" width="100" fixed="right">
                <template #default="scope">
                    <el-button @click="viewSessionDetail(scope.row)" text type="primary">查看详情</el-button>
                </template>
            </el-table-column>
        </el-table>
...
// 使用辅导员专用接口：返回全部学生的日记，且按心情分升序（状态最差的排前面）
import { getCounselorDiaries } from '@/api/admin'
...
// 原筛选条件（用户ID / 情绪评分区间）已移除：
// 服务端未实现对应过滤，保留一个不起作用的筛选栏比没有更糟——
// 用户输入条件后看到结果没变化，会误以为是数据问题。
// 如需筛选，应在 /emotion-diary/counselor/page 上补充参数并在此恢复。
...
const handleSearch = async () => {
    const { records, total } = await getCounselorDiaries({
...
// 说明：这里刻意不提供「删除」操作。
// 情绪日记是学生的私人记录，辅导员承担的是干预职责，不应具备销毁学生数据的权限。
// 需要清理数据时应由管理员在数据库层面处理，并留下操作记录。
