...
const submitForm = async (formEl) => {
    ...
    try {
        ...
        // 成功时响应拦截器返回用户详情对象（不含 code 字段）
        if (res?.data?.code) {
            // 后端业务/参数错误（拦截器未拦截的错误码）
            ElMessage.error(res.data.data || res.data.msg || '注册失败，请检查填写的信息')
        } else {
        ...
        ElMessage.error(e?.response?.data?.msg || '注册失败，请检查填写的信息')
    }
}
</script>
